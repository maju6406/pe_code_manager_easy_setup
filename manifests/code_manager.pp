...
class pe_code_manager_easy_setup::code_manager (
  ...
  String  $git_management_system            = 'github',
  ...
  exec { 'pe_code_manager_easy_setup chown all environments to pe-puppet' :
  ...
  exec { "Generate Token for ${code_manager_service_user}" :
    command => epp('pe_code_manager_easy_setup/code_manager/create_rbac_token.epp',
  ...
  $webhook_url = "https://${::fqdn}:8170/code-manager/v1/webhook?type=${git_management_system}${token_info}"
